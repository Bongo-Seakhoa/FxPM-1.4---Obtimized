...
var INDEX_PREFIXES = ["US30", "US100", "US500", "DE30", "EU50", "UK100", "JP225", "FR40", "ES35", "HK50", "AU200"];
var CRYPTO_PREFIXES = ["BTC", "ETH", "XRP", "TON", "LTC", "SOL", "BCH", "DOG", "TRX"];
var ENERGY_PREFIXES = ["XTI", "XBR", "XNG"];
...
function renderTable() {
  ...
  for (var i = 0; i < rows.length; i += 1) {
    ...
    row.innerHTML =
      ...
      "<td>" + PMCommon.escapeHtml(entry.reason || "N/A") + "</td>" +
...
function defaultPipSize(symbol) {
  ...
  if (upper.indexOf("XTI") === 0 || upper.indexOf("XBR") === 0) return 0.01;
  if (upper.indexOf("XNG") === 0) return 0.001;
  if (startsWithAny(upper, INDEX_PREFIXES)) return 1;
  if (startsWithAny(upper, CRYPTO_PREFIXES)) return 1;
  return 0.0001;
}
...
function defaultPipValue(symbol) {
  ...
  if (startsWithAny(upper, ENERGY_PREFIXES)) return 1;
  if (startsWithAny(upper, INDEX_PREFIXES)) return 1;
  if (startsWithAny(upper, CRYPTO_PREFIXES)) return 1;
  return 10;
}
...
function renderDetails(entry) {
  ...
  var details = [
    ...
    ["Secondary", entry.secondary_trade === null || entry.secondary_trade === undefined ? "N/A" : (entry.secondary_trade ? "YES" : "NO")],
