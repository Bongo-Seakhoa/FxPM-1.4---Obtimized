...
`config.json` is the primary PM configuration file. The committed file in this repo currently reflects a lower-risk live/challenge-style profile, so review it before deployment.
...
SAVED EURUSD to pm_configs.json
```
...
The optimization ledger is stateful and atomic. If the process is interrupted, rerun the same command and the PM resumes from existing saved state unless `--overwrite` is used.
...
- The PM refreshes bars per symbol/timeframe
- It computes features and the active regime
- It looks up the exact validated winner for `(timeframe, regime)`
- It applies risk sizing, broker constraints, and spread-quality checks
- It sends orders through MT5 if a trade passes all gates

### Live safety notes

- The PM is candle-driven, so trades occur on the next actionable bar after the signal decision.
...
- `pm_configs.json` has been regenerated or confirmed current
...
- can optionally trigger root `data/` maintenance jobs through the API
...
| `pm_configs.json` | Saved validated winners |
